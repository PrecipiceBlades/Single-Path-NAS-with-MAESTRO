...
def get_conv_output_and_params_and_flops(input_size, layer_type, kernel_size, stride, expansion=1, out_channels=1, use_bn=True, use_act=True):
    ...
    if layer_type == "DWConv":
        model = ConvBlock(inputs.size()[1], inputs.size()[1], "DWConv", kernel_size, stride, use_bn=use_bn, use_act=use_act)
    elif layer_type == "SepConv":
        model = ConvBlock(inputs.size()[1], out_channels, "SepConv", kernel_size, stride, use_bn=use_bn, use_act=use_act)
    else:
        model = ConvBlock(inputs.size()[1], int(out_channels*expansion), "Conv", kernel_size, stride, use_bn=use_bn, use_act=use_act)
...
def get_linear_output_size_and_nb_param(input_size, out_features, bias=True, use_pool=True):
    ...
    if use_pool:
        ...
        model = LinearBlock(in_features, out_features, bias=bias)
...
def make_summary(profiled_layers, dataflow, model_name):
    ...
    # Save the dataflow
    outfile = model_name + "_"+ dataflow + ".m"
    with open("dataflow/" + dataflow + ".m", "r") as fd:
        with open("dpt.m", "r") as fdpt:
            with open("../DFSL_description/" + outfile, "w") as fo:
